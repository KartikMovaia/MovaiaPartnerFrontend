...
- **Dashboards are date-range configurable** — `shared/ui/DateRangePicker` + `shared/utils/dateRange`; `analyticsService.overview` / `adminAnalyticsService.partnersOverview` take `{from?,to?}`. KPIs + trend reflect the range (default **all time**); the dashboard's **recent-scans** table is intentionally **not** range-filtered (always the latest 50). Data fetches on mount + on range change only — no polling/focus refetch.
- **Full scan log is its own page** — `/partner/scans` (`partner-admin/src/pages/Scans.tsx`), shared by partner + outlet admins. Filter by branch (partner admin only; `storeService.listBranches` → `GET /stores`, so PARTNER_ADMIN-only), date, status, and debounced customer search (name/email/phone); server-side pagination; CSV export (client-side, all matching rows, formula-injection-guarded). `analyticsService.scans` takes `{page,pageSize,storeId?,status?,from?,to?,search?}`. Outlet admins are auto-scoped to their store server-side. Linked from the dashboard's recent-scans "View all →".
