...
- **Full scan log is its own page** — `/partner/scans` (`partner-admin/src/pages/Scans.tsx`), shared by partner + outlet admins. Filter by branch (partner admin only; `storeService.listBranches` → `GET /stores`, so PARTNER_ADMIN-only), date, and status; server-side pagination; CSV export (client-side, all matching rows, formula-injection-guarded). `analyticsService.scans` takes `{page,pageSize,storeId?,status?,from?,to?}`. Outlet admins are auto-scoped to their store server-side. Linked from the dashboard's recent-scans "View all →".
